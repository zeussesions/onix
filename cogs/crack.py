...
class crack(commands.Cog):
    ...
    @commands.hybrid_command(name='crack', with_app_command=True)
    async def crack(self, ctx: commands.Context, code: str):
        if len(code) != 4:
...
async def setup(bot):
    await bot.add_cog(crack(bot), guilds=[discord.Object(id=874842871801479208)])
